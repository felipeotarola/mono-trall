...
export async function POST(request: Request) {
  ...
  const generatedImages = demoMode
    ? getDemoGeneratedImages(payload, input)
    : await uploadGeneratedImages(payload, input, auth!.userId)
...
function getDemoReferenceImages(input: VisualizationRequest) {
...
function getDemoGeneratedImages(
  payload: OpenAIImageResponse,
  input: VisualizationRequest
) {
  const base64Images =
